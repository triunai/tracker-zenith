...
function App() {
  ...
  useEffect(() => {
    ...
    let authOperationTimeout: NodeJS.Timeout | null = null; // Initialize to null
    ...
    const observer = new MutationObserver((mutations) => {
      ...
      if (isAuthLoading && !authOperationStarted) {
        ...
        authOperationTimeout = setTimeout(() => {
          ...
          if (window.confirm('Authentication is taking longer than expected. Would you like to reset and try again?')) {
        ...
        if (authOperationTimeout) {
          ...
          authOperationTimeout = null; // Reset timeout ID
        }
      }
    });
